	...
	<form:form id="searchForm" modelAttribute="oaNotify" action="${ctx}/oa/oaNotify/${oaNotify.self?'self':''}" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<c:if test="${!requestScope.oaNotify.self}"><li><label>状态：</label>
				<form:radiobuttons path="state" items="${fns:getDictList('oa_notify_state')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
			</li></c:if>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="oaNotify">
			<tr>
				...
				<td>
					${fns:getDictLabel(oaNotify.state, 'oa_notify_state', '')}
				</td>
				<td>
