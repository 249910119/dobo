...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/oa/oaNotify/form?id=${oaNotify.id}">通知<shiro:hasPermission name="oa:oaNotify:edit">${oaNotify.state eq '1' ? '查看' : not empty oaNotify.id ? '修改' : '添加'}</shiro:hasPermission><shiro:lacksPermission name="oa:oaNotify:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="oaNotify" action="${ctx}/oa/oaNotify/save" method="post" class="form-horizontal">
		...
		<c:if test="${oaNotify.state ne '1'}">
			...
			<div class="control-group">
				...
				<div class="controls">
					<form:radiobuttons path="state" items="${fns:getDictList('oa_notify_state')}" itemLabel="label" itemValue="value" htmlEscape="false" class="required"/>
