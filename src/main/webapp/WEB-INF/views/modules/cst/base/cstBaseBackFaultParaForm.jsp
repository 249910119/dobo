...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/cst/base/cstBaseBackFaultPara/">备件故障率参数获取列表</a></li>
		<li class="active"><a href="${ctx}/cst/base/cstBaseBackFaultPara/form?id=${cstBaseBackFaultPara.id}">备件故障率参数获取<shiro:hasPermission name="cst:base:cstBaseBackFaultPara:edit">${not empty cstBaseBackFaultPara.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="cst:base:cstBaseBackFaultPara:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="cstBaseBackFaultPara" action="${ctx}/cst/base/cstBaseBackFaultPara/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="resstattypeName" htmlEscape="false" maxlength="64" class="input-xlarge "/>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			<label class="control-label">在保数量：</label>
			<div class="controls">
				<form:input path="onlineAmount" htmlEscape="false" class="input-xlarge  number"/>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">更换数量：</label>
			<div class="controls">
				<form:input path="changeAmount" htmlEscape="false" class="input-xlarge  number"/>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">备件故障率：</label>
			<div class="controls">
				<form:input path="backFaultRate" htmlEscape="false" class="input-xlarge  number"/>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">平均故障成本：</label>
			<div class="controls">
				<form:input path="averFaultCost" htmlEscape="false" class="input-xlarge  number"/>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
				<input name="statusChangeDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate "
					value="<fmt:formatDate value="${cstBaseBackFaultPara.statusChangeDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
		...
		<div class="form-actions">
			<shiro:hasPermission name="cst:base:cstBaseBackFaultPara:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
