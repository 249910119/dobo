...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/fc/fcPlanInnerFee/">项目计划内财务费用列表</a></li>
		<li class="active"><a href="${ctx}/fc/fcPlanInnerFee/form?id=${fcPlanInnerFee.id}">项目计划内财务费用<shiro:hasPermission name="fc:fcPlanInnerFee:edit">${not empty fcPlanInnerFee.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="fc:fcPlanInnerFee:edit">查看</shiro:lacksPermission></a></li>
	...
	<form:form id="inputForm" modelAttribute="fcPlanInnerFee" action="${ctx}/fc/fcPlanInnerFee/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<sys:message content="${message}"/>			
		<div class="control-group">
			<label class="control-label">项目信息：</label>
			<div class="controls">
				<form:hidden id="fcProjectInfoId" path="fcProjectInfoId" value="${fcPlanInnerFee.fcProjectInfo.id}" htmlEscape="false" maxlength="64" />
				<input type="text" id="fcProjectInfoCode" name="fcProjectInfoCode" value="${fcPlanInnerFee.fcProjectInfo.projectCode}" maxlength="64" class="input-medium required" readonly="readonly"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<input name="calculateTime" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate required"
					value="<fmt:formatDate value="${fcPlanInnerFee.calculateTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"
		...
		<div class="control-group">
			<label class="control-label">传入参数：</label>
			<div class="controls">
				<form:textarea path="calcParaJson" htmlEscape="false" rows="4" maxlength="4000" class="input-xxlarge "/>
			</div>
		</div>
		<div class="control-group">
		...
		<c:if test="${not fcPlanInnerFee.isNewRecord}">
		<div class="control-group">
		...
		<div class="form-actions">
			<c:if test="${fcPlanInnerFee.isNewRecord or fcPlanInnerFee.status eq 'A0'}">
			<shiro:hasPermission name="fc:fcPlanInnerFee:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
			</c:if>
