 ...
 *
 */
public class ResidentFavorCostImpl extends CalCostService {
	
	// 折减对应的资源计划 
	private String[] favorLevel = new String[]{"MANCSTHZ1","MANCSTHZ2","MANCSTHZ3","MANCSTHZ4","MANCSTHZ5","MANCSTHZ6"};
    ...
    public void cost(CstModelProdModuleRel module, Map<String, Map<String, ProdDetailInfo>> detailInfoMap, Map<String, Map<String, DetailCostInfo>> proddciMap) {
		...
		for(String cityAndRestype : residentFavorSplit.getResidentDetailResMap().keySet()) {
			...
			// 折减顺序：(一线2级)→(一线3级)→(一线4级)→(一线5级)→(一线6级)
			String[] residentLevel = new String[]{"MANCSTH12","MANCSTH13","MANCSTH14","MANCSTH15","MANCSTH16"};
			// 折减故障和巡检服务中的当前级别
			int level = 0;
			...
			/*// 20190409:驻场折减规则-同省，同分类下驻场折减对应的故障和巡检产品的资源
			Double residentResAmount = 0.0;
			for(String resideLevel : residentResMap.keySet()) {
				residentResAmount = residentResAmount + residentResMap.get(resideLevel);
			}
			
			if(residentResAmount > 0) {
				for(int i=6;i>0;i--) {
			...
			for(String rl : residentLevel) {
				...
				level = Integer.valueOf(rl.substring(rl.length()-1));  // 工程师级别
				
				// 20190408 驻场工程师折减规则调整：工程师可以折减同级别+1 的资源
				for(int i=level+1;i>0;i--) {
				//for(int i=level;i>0;i--) {  // 20190408调整前规则
					measureId = "MANCSTH1" + i;
					favorMeasureId = "MANCSTHZ" + i;
					if(prodResMap.get(measureId) == null) continue;
    ...
    public void calculateFormulaForCost(CstModelProdModuleRel module, DetailCostInfo dcis) {
		...
		for(CstModelCalculateFormula cmcf : pmrMap.values()) {
			if("MANCSTHZ1,MANCSTHZ2,MANCSTHZ3,MANCSTHZ4,MANCSTHZ5,MANCSTHZ6".contains(cmcf.getMeasureId())) continue;
	...
	public void splitDetailsByCityAndResType(String prodId, Map<String, Map<String, ProdDetailInfo>> detailInfoMap, Map<String, Map<String, DetailCostInfo>> proddciMap) {
		...
		Map<String, CstCheckResstatSystemRel> ccrsrMap = CacheDataUtils.getCstCheckResstatSystemRelMap();
		String cityId ;
		String resstattyeId ;
		...
		for(ProdDetailInfo pdi : detailInfoMap.get(prodId).values()) {
			...
			if(cstDictPara.getParaValue() > 0) {
				...
				cityId = cmcpMap.get(cityId).getProvince();
				...
				resstattyeId = ccrsrMap.get(resstattyeId).getDescTypeName();
				// 驻场的级别
				residentLevel = CacheDataUtils.getCstManLevelRoleRelNewMap().get(cstResourceBaseInfo.getLaborLevel()+cstResourceBaseInfo.getResstattypeName()).getMeasureId();
		...
		if(residentFavorSplit.getResidentDetailResMap().size() > 0) {
			...
			String[] line1LevelMeasure = new String[]{"MANCSTH11","MANCSTH12","MANCSTH13","MANCSTH14","MANCSTH15","MANCSTH16"};
