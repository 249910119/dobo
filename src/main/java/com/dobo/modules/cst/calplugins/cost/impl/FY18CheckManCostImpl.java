 ...
 *
 */
public class FY18CheckManCostImpl extends CalCostService {
	// 巡检矩阵拆分用到的实体类
    ...
    public void cost(CstModelProdModuleRel module, ProdDetailInfo detailInfo, DetailCostInfo dcis) {
		...
		try {
			...
			// pmoAndResNHour	pmo、资源岗人工资源计划'--现场巡检（单位：人月）
			String pmoAndResNHour = "(0";
			// pmoAndResFHour	pmo、资源岗人工资源计划'--远程巡检（单位：人月）
			String pmoAndResFHour = "(0";
			// pmoAndResDHour	pmo、资源岗人工资源计划'--深度巡检（单位：人月）
			String pmoAndResDHour = "(0";
			// line1TravelNHour	一线人工差旅资源计划'--现场巡检（单位：人月） 
			String line1TravelNHour = "(0";
			// line1TravelFHour	一线人工差旅资源计划'--远程巡检（单位：人月）
			String line1TravelFHour = "(0";
			// line1TravelDHour	一线人工差旅资源计划'--深度巡检（单位：人月）
			String line1TravelDHour = "(0";
			...
			for(String check : detailInfo.getMonthCheckNumMap().keySet()) {
				...
				for(String yyyyMM : detailInfo.getMonthCheckNumMap().get(check).keySet()) {
					...
					// m月清单中 不同城市不同设备大类的台次之和
					String splitMonthSum = "(0";
					for(Double num : checkDetailSplit.getSplitMonthMap().get(check).get(yyyyMM).values()) {
						splitMonthSum += "+" + num ;
					}
					splitMonthSum += ")";
					...
					if("BUYCHECKN".equals(check)) {
						...
						// pmo、资源岗人工资源计划=∑m（COUNT（城市&设备大类），并按照清单维度拆分
						pmoAndResNHour += "+" + checkDetailSplit.getSplitMonthMap().get(check).get(yyyyMM).size() +
										  "*" + currMonthAmount + "/" + splitMonthSum;
						// 一线人工差旅资源计划'（单位：人月）=差旅工时④/8/250*12 ，并按照同订单、同城市、同设备大类拆分
						line1TravelNHour += "+" + currMonthAmount + "/" + currMonthSum;
					...
					// 远程巡检
						...
						// pmo、资源岗人工资源计划=∑m（COUNT（城市&设备大类），并按照清单维度拆分
						pmoAndResFHour += "+" + checkDetailSplit.getSplitMonthMap().get(check).get(yyyyMM).size() +
										  "*" + currMonthAmount + "/" + splitMonthSum;
						/*// 一线人工差旅资源计划'（单位：人月）=差旅工时④/8/250*12 ，并按照同订单、同城市、同设备大类拆分
						...
						// 远程巡检差旅资源计划和差旅费为0
						line1TravelFHour += "0";  // 一线人工差旅资源计划'（单位：人月）=差旅工时④/8/250*12 ，并按照同订单、同城市、同设备大类拆分
					...
					// 深度巡检
						...
						// pmo、资源岗人工资源计划=∑m（COUNT（城市&设备大类），并按照清单维度拆分
						pmoAndResDHour += "+" + checkDetailSplit.getSplitMonthMap().get(check).get(yyyyMM).size() +
										  "*" + currMonthAmount + "/" + splitMonthSum;
						// 一线人工差旅资源计划'（单位：人月）=差旅工时④/8/250*12 ，并按照同订单、同城市、同设备大类拆分
						line1TravelFHour += "+" + currMonthAmount + "/" + currMonthSum;
						// 一线差旅费'=（差旅费④-住宿费及其他④）+ROUND（（∑n（γmn*单台设备巡检工时⑧）)/8，0）*住宿费及其他④ ，并按照同订单、同城市、同设备大类拆分
			...
			pmoAndResNHour += ")";
			paraMap.put("pmoAndResNHour", pmoAndResNHour);
			pmoAndResFHour += ")";
			paraMap.put("pmoAndResFHour", pmoAndResFHour);
			pmoAndResDHour += ")";
			paraMap.put("pmoAndResDHour", pmoAndResDHour);

			line1TravelNHour += ")*" + paraMap.get("travelHour") + "/8/250*12";
			paraMap.put("line1TravelNHour", line1TravelNHour);
			line1TravelFHour += ")*" + paraMap.get("travelHour") + "/8/250*12";
			paraMap.put("line1TravelFHour", line1TravelFHour);
			line1TravelDHour += ")*" + paraMap.get("travelHour") + "/8/250*12";
			paraMap.put("line1TravelDHour", line1TravelDHour);
