 ...
 *
 */
public class ManSpecialCostImpl extends CalCostService {
    ...
    public void cost(CstModelProdModuleRel module, ProdDetailInfo detailInfo, DetailCostInfo dcis) {
		...
		try {
			...
			for(CstModelCalculateFormula cmcf : pmrMap.values()) {
				...
				if("MANCSTH12,MANCSTH13,MANCSTH14,MANCSTH15,MANCSTHG4,MANCSTHG5".contains(cmcf.getMeasureId())) {
					if(cmcf.getMeasureId().equals(CacheDataUtils.getCstManLevelRoleRelNewMap().get(paraMap.get("level")+paraMap.get("resstattype")).getMeasureId())) {
