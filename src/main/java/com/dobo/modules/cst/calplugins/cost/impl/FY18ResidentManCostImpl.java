 ...
 *
 */
public class FY18ResidentManCostImpl extends CalCostService {
    ...
    public void cost(CstModelProdModuleRel module, ProdDetailInfo detailInfo, DetailCostInfo dcis) {
		...
		try {
			...
			for(CstModelCalculateFormula cmcf : pmrMap.values()) {
				...
				// 根据明细中工程师的级别，将非对应的角色级别设置为0   （高级    一线2级;  中级   一线3级;  初级   一线4级 ）
				if("MANCSTH12,MANCSTH13,MANCSTH14".contains(cmcf.getMeasureId()) &&
						!cmcf.getMeasureId().equals(CacheDataUtils.getCstManLevelRoleRelMap().get(paraMap.get("level")).getMeasureId())) {
					calValue = "0.0";
				} else {
